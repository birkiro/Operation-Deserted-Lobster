...
static ssize_t dac_read(struct file *filp, char *buf, size_t count, loff_t *offp)
{
	...
	if(i2c_master_recv(dac_i2c_client, data_string, 6))
	{
		...
		valprobe = valprobe*330/4096;
		printk(KERN_INFO "dac: current output voltage is %d.%d%dv.", valprobe/100, valprobe%100/10, valprobe%10);
...
static ssize_t dac_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos )
{
	int status = 1;
	unsigned char i,j;
	int _temp_buff[3]; //adapt the userspace input into dac raw value;
	int valraw = 0, val = 0;
	...
	if((dac_dev.user_buff[0] >= 0x30 && dac_dev.user_buff[0] <= 0x33) && dac_dev.user_buff[1] == '.' &&
	   ...
	   (dac_dev.user_buff[3] >= 0x30 && dac_dev.user_buff[3] <= 0x39) && dac_dev.user_buff[4] == 'v')
	{
		for(i=1; i<4; i++)
		{
		...
		valraw = _temp_buff[0]*1000+_temp_buff[1]*100+_temp_buff[2]*10;
