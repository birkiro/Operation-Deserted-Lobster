...
#define MY_GPIO 60 //bank 1 = 32 + 28 = gpio1_28 
...
static ssize_t dulldev_read( struct file* F, char *buf, size_t count, loff_t *f_pos )
{
	...
	count = sizeof( buffer );
  	...
  	printk(KERN_INFO "dulldev: Read gpio status. Status: %d", gpio_value);
...
static ssize_t dulldev_write( struct file* F, const char *buf, size_t count, loff_t *f_pos )
{
	...
	if(copy_from_user(dulldev.status, buf, 4)) {
...
static int init_dulldev(void)
{
	...
	/* The kernel uses structures of type struct cdev to represent char devices internally. 	 * Before the kernel invokes our device's operations, we must allocate and register one or 		 * more of these structures. To do so, the code must include <linux/cdev.h>, where the 		 * structure and its associated helper functions are defined.
	 * 
	 * In our example here we want to embed the cdev structure within a device-specific structure 		 * of your own. In this case we initialize the structure that we have already allocated.  
	 */
	...
	printk(KERN_ALERT "dulldev: Successfully allocated memory at address %08x.\n", dulldev.status);

    //b4me hack for gpio 
	err = gpio_request(MY_GPIO, "my_gpio");
	if (err < 0) {
		printk(KERN_ALERT DEVICE_NAME " : failed to request GPIO pin\n");
		goto fail;
	}
	err = gpio_direction_input(MY_GPIO);
	if (err < 0) {
		printk(KERN_ALERT DEVICE_NAME " : failed to set GPIO as input\n");
		goto fail;
	}
